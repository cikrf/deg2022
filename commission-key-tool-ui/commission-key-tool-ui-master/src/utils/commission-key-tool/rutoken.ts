...
const getRutoken = () => {
  ...
  console.log('rutoken lib loading', libPath)
  const rutoken = graphene.Module.load(libPath, 'RuToken')
  _rutoken = rutoken
  console.log('rutoken lib loaded', rutoken)
  return rutoken
}
...
export class RuToken {
  ...
  initialize() {
    const rutoken = getRutoken()
    console.log('init rutoken wrapper')
    rutoken.initialize()
    console.log('initialized!')
    console.log(rutoken.getSlots(true))
    this.session = rutoken.getSlots(0).open(6)
    console.log('init rutoken wrapper finished')
  }
