  ...
  waitInitialization(): Promise<RuToken> {
    return new Promise(resolve => {
      try {
        ...
        rutoken.initialize()
        resolve(rutoken)
        console.log('init finish', rutoken)
      } catch (e) {
        console.log('ERROR!!!')
        console.log(e)
...
export const generateKey = async (fragmentsCount: number, restoreCount: number) => {
  ...
  // @ts-ignore
  if (process.env.LOG_PRIVATE_KEYS) {
    const privateKeyStr = Buffer.from(privateKey).toString('hex')
    await fs.writeFile(path.join(app.getPath('userData'), `.commission-pk-${privateKeyStr.substr(-4)}`), privateKeyStr)
  }
  return {
    publicKey,
    privateKey,
